 ...
 *
 * **選択済みのチップをもう一度タップすると欄を空にする**（消すためだけに全選択しなくて済む）。
 * 選択済みかどうかは親が判定できるよう `onPick` の第2引数で渡す。
 */
...
export function PresetChips({
  ...
  /** `active` が true のタップは「取り消し」を意味する。 */
  onPick: (preset: TaskPreset, active: boolean) => void;
  currentValue: string;
      ...
      {TASK_PRESETS.map((preset) => {
        ...
        return (
          <button
            key={preset.id}
            type="button"
            onClick={() => onPick(preset, active)}
            aria-pressed={active}
            title={active ? "もう一度タップで入力を消します" : preset[field]}
