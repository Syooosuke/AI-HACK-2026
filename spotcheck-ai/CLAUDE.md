...
| D-05 | 受注人数 | **1依頼に対し依頼者が撮影人数 N を指定**し、N人まで受注可能。**自分が出した依頼は自分で受注できない** |
| D-06 | 認証 | **ログインID＋パスワード**で認証する。パスワードは bcrypt でハッシュ化し、JWT（HS256・既定30日）を `Authorization: Bearer` で受け取る。**users に role は持たせない**（1アカウントで「依頼する」「撮影する」の両方を行う）。権限は依頼のオーナーか受注者かで判定する |
...
- 色・余白は Tailwind のデザイントークン（`docs/05-frontend.md` の定義）に従う。役割ではなく文脈で使い分け、依頼する側の操作は青系、撮影する側の操作は緑系とする。
- ログイン後の画面構成は全ユーザー共通。下部タブは ホーム / さがす / 依頼する / お知らせ / マイページ。
...
> **`main` にマージされたものが、そのまま本番のURLになる**（GitHub Actions が自動デプロイする）。
> `dev` → `main` のマージは「リリース」そのものであり、取り消しには再デプロイが要る。
> だから人間の確認が要る。詳細は `docs/07-deployment.md` 0節。
>
> 本番へ手元から直接デプロイしない（`deploy/deploy.sh` は障害時の退避手段）。
> 手元から流すと `main` と本番の中身がずれ、「何が動いているか」が追えなくなる。
