...
- ワーカーが重大な危険にさらされる場所（立入禁止区域、災害現場、高速道路など）への立ち入りを要するもの
- 第三者との深刻なトラブルを誘発する可能性が極めて高いもの（私有地内への侵入、直接的な声かけを要するもの）

【妥当性審査 validity】明らかに実行不可能（存在しない場所、視認不可能な対象など）でなければ "pass" とします。
...
- validity が "fail"、または score が60未満 → "needs_info"
- それ以外 → "approved"
