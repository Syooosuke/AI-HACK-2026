...
**1回目はスキーマ強制（`response_format`）を付けない。** 解析に失敗したときだけ、
次の試行から付ける。

実測（claude-opus-5・同一画像・同一プロンプト）:

| 条件 | 所要時間 | 判定 |
|---|---|---|
| `response_format` あり | 58.3秒 / 77.8秒 | score 68 / 58 |
| `response_format` なし | **6.3秒 / 6.5秒** | score 62 / 55 |

**約10倍遅くなる一方、判定の中身は変わらなかった。**
現地で待つワーカーの時間を「起きるかもしれない解析失敗」のために毎回犠牲にする
理由はないため、失敗してから使う。
...
| 種類 | 例 | 考え方 |
|---|---|---|
| 単語1つで却下 | 盗撮 / ストーカー / 空き巣 / 侵入経路 | 言い逃れの余地が無いもの |
| 2語の共起で却下 | 「施錠」＋「確認」、「行動パターン」＋「元カノ」 | 片方だけでは正当な依頼を巻き込むため |
